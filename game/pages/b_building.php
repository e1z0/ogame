...
function t() {
	...
	if ((ss + 3) < aa) {
	  bxx.innerHTML="<?=loca("B_BUILDING_DONE");?><br>"+"<a href=index.php?page=b_building&session="+ps+"&planet="+pl+"><?=loca("B_BUILDING_NEXT");?></a>";
	...
	if(s < 0) {
        ...
        if (1) {
        	bxx.innerHTML=h+":"+m+":"+s+"<br><a href=index.php?page=b_building&session="+ps+"&listid="+pk+"&modus="+pm+"&planet="+pl+"><?=loca("B_BUILDING_CANCEL");?></a>";
    	} else {
    		bxx.innerHTML=h+":"+m+":"+s+"<br><a href=index.php?page=b_building&session="+ps+"&listid="+pk+"&modus="+pm+"&planet="+pl+"><?=loca("B_BUILDING_CANCEL");?></a>";
    	}
	}    
...
for ( $i=0; $i<$cnt; $i++ )
{
    ...
    if ( $prem['commander'] )
    {
        ...
        //FIXME
        if ($queue['level'] > 0) echo " , ".loca("B_BUILDING_LEVEL")." ".$queue['level'];
        if ( $queue['destroy'] ) echo "\n ".loca("B_BUILDING_DESTROY");
        if ($i==0) {
            ...
            echo "<td class=\"k\"><font color=\"red\"><a href=\"index.php?page=b_building&session=$session&modus=remove&listid=".$queue['list_id']."&planet=".$aktplanet['planet_id']."\">".loca("B_BUILDING_REMOVE")."</a></font></td></td></tr>\n";
        }
    }
}
...
foreach ( $buildmap as $i => $id )
{
    ...
    if ( $lvl ) echo " (".loca("B_BUILDING_LEVEL")." ".$lvl.")";
    echo "<br>". loca("SHORT_$id");
    $res = BuildPrice ( $id, $lvl+1 );
    $m = $res['m']; $k = $res['k']; $d = $res['d']; $e = $res['e'];
    echo "<br>".loca("B_BUILDING_PRICE").":";
    ...
    echo "<br>".loca("B_BUILDING_DURATION").": ".BuildDurationFormat ( $t )."<br>";

    if ( $prem['commander'] ) {
        if ( $cnt ) {
            if ( $cnt < 5) echo "<td class=k><a href=\"index.php?page=b_building&session=$session&modus=add&techid=$id&planet=".$aktplanet['planet_id']."\">".loca("B_BUILDING_TO_QUEUE")."</a></td>";
            else echo "<td class=k>";
        }
        else
        {
                  if ( $aktplanet['fields'] >= $aktplanet['maxfields'] ) {
                        echo "<td class=l><font color=#FF0000>".loca("B_BUILDING_QUEUE_NO_SPACE")." </font>";
                  }
                  else if ( $id == 31 && $reslab_operating ) {
				echo "<td class=l><font  color=#FF0000>".loca("B_BUILDING_QUEUE_IN_PROCESS")."</font> <br>";
			}
      		...
      		{
        		if ( IsEnoughResources ( $aktplanet, $m, $k, $d, $e )) echo "<td class=l><a href='index.php?page=b_building&session=$session&modus=add&techid=".$id."&planet=".$aktplanet['planet_id']."'><font color=#00FF00> ".loca("B_BUILDING_BUILD")." </font></a>\n";
          		else echo "<td class=l><font color=#FF0000> ".loca("B_BUILDING_BUILD")." </font>\n";
			}
   			else
      		{
        		if ( IsEnoughResources ( $aktplanet, $m, $k, $d, $e )) echo "<td class=l><a href='index.php?page=b_building&session=$session&modus=add&techid=".$id."&planet=".$aktplanet['planet_id']."'><font color=#00FF00>".loca("B_BUILDING_UPGRADE_TO")."  ".($lvl+1)."</font></a>\n";
          		else echo "<td class=l><font color=#FF0000>".loca("B_BUILDING_UPGRADE_TO")."  ".($lvl+1)."</font>";
			}
        }
    }
    else {
