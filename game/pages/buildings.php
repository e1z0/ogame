...
loca_add ( "b_building", $GlobalUser['lang'] );
loca_add ( "buildings", $GlobalUser['lang'] );
...
echo "<title> \n";
echo loca("BUILDINGS_BUILDINGS")."#Gebaeude\n";
echo "</title> \n";
...
if ( $_GET['mode'] === "Flotte" )
{
    ...
    if ( $busy ) {
        echo "<br><br><font color=#FF0000>".loca("BUILDINGS_NANITES_PERFECTED")."</font><br><br>";
    }
    if ( $GlobalUser['vacation'] ) {
        echo "<font color=#FF0000><center>".loca("B_BUILDING_VACATION_UNTIL")."  ".date ("Y-m-d H:i:s", $GlobalUser['vacation_until'])."</center></font>";
    }
    ...
    echo "         <tr> \n";
    echo "          <td class=l colspan=\"2\">".loca("BUILDINGS_DESCRIPTION")."</td> \n";
    echo "          <td class=l><b>".loca("BUILDINGS_NO")."</b></td> \n";
    echo "          </tr> \n\n";
    ...
    if ( $aktplanet['b21'] ) {
        ...
        foreach ( $fleetmap as $i => $id ) {
            ...
            if ($aktplanet['f'.$id]) echo "</a> (".loca("BUILDINGS_IN_STOCK")." ".$aktplanet['f'.$id].")";
            $res = ShipyardPrice ( $id );
            $m = $res['m']; $k = $res['k']; $d = $res['d']; $e = $res['e'];
            echo "<br>".loca("SHORT_$id")."<br>".loca("B_BUILDING_PRICE").":";
            if ($m) echo " ".loca("B_BUILDING_PRICE_METAL").": <b>".nicenum($m)."</b>";
            if ($k) echo " ".loca("B_BUILDING_PRICE_CRYSTAL").": <b>".nicenum($k)."</b>";
            if ($d) echo " ".loca("B_BUILDING_PRICE_DEUTERIUM").": <b>".nicenum($d)."</b>";
            if ($e) echo " ".loca("B_BUILDING_PRICE_ENERGY").": <b>".nicenum($e)."</b>";
            $t = ShipyardDuration ( $id, $aktplanet['b21'], $aktplanet['b15'], $speed );
            echo "<br>".loca("B_BUILDING_DURATION").": ".BuildDurationFormat ( $t )."<br></th>";
            echo "<td class=k >";
            if ( !ShipyardMeetRequirement ( $GlobalUser, $aktplanet, $id ) ) echo "<font color=#FF0000>".loca("BUILDINGS_IMPOSSIBLE")."</font>";
        ...
        echo "<td class=c colspan=2 align=center><input type=submit value=\"".loca("B_BUILDING_BUILD")."\"></td></tr>";
    }
    else {
        if (!$busy) echo "<table><tr><td class=c>".loca("BUILDINGS_SHIPYARD_REQUIRED")."</td></tr></table>";
    }
}
...
if ( $_GET['mode'] === "Verteidigung" )
{
    ...
    if ( $aktplanet['b21'] ) {
        ...
        foreach ( $defmap as $i => $id ) {
            ...
            echo "<a href=index.php?page=infos&session=$session&gid=$id>".loca("NAME_$id")."</a>";
            if ($aktplanet['d'.$id]) echo "</a> (".loca("BUILDINGS_IN_STOCK")." ".$aktplanet['d'.$id].")";
            $res = ShipyardPrice ( $id );
            $m = $res['m']; $k = $res['k']; $d = $res['d']; $e = $res['e'];
            echo "<br>".loca("SHORT_$id")."<br>".loca("B_BUILDING_PRICE").":";
            if ($m) echo " ".loca("B_BUILDING_PRICE_METAL").": <b>".nicenum($m)."</b>";
            ...
            echo "<td class=k >";
            if ( !$busy ) {
                if ( ($id == 407 || $id == 408) && $aktplanet['d'.$id] > 0 ) echo "<font color=#FF0000>".loca("BUILDINGS_SHIELD_DOME_1")."</font>";
                else if ( !ShipyardMeetRequirement ( $GlobalUser, $aktplanet, $id ) ) echo "<font color=#FF0000>".loca("BUILDINGS_IMPOSSIBLE")."</font>";
...
if ( $_GET['mode'] === "Forschung" )
{
    ...
    if ( $busy ) {
        echo "<br><br><font color=#FF0000>".loca("BUILDINGS_RESEARCH_BUILDING_UNKNOWN_REQUIRED")."</font><br /><br />";
    }
    if ( $GlobalUser['vacation'] ) {
        echo "<font color=#FF0000><center>".loca("B_BUILDING_VACATION_UNTIL")."  ".date ("Y-m-d H:i:s", $GlobalUser['vacation_until'])."</center></font>";
    }
    ...
    if ( $aktplanet['b31'] ) {
        ...
        foreach ( $resmap as $i => $id ) {
            ...
            echo "<a href=index.php?page=infos&session=$session&gid=$id>".loca("NAME_$id")."</a>";
            if ($GlobalUser['r'.$id]) echo "</a> (".loca("B_BUILDING_LEVEL")." ".$GlobalUser['r'.$id];
            if ( $id == 106 && $prem['technocrat'] ) { 
                echo " <b><font style=\"color:lime;\">+2</font></b> <img border=\"0\" src=\"img/technokrat_ikon.gif\" alt=\"".loca("BUILDINGS_TECHNOCRAT")."\" onmouseover=\"return overlib('<font color=white>".loca("BUILDINGS_TECHNOCRAT")."</font>', WIDTH, 100);\" onmouseout='return nd();' width=\"20\" height=\"20\" style=\"vertical-align:middle;\"> ";
            }
...
?>
                ...
                function t(){
                    ...
                    if(s<0){
                        ...
                        bxx.innerHTML='<?=loca("B_BUILDING_DONE");?><br><a href=index.php?page=buildings&session=<?=$session;?>&mode=Forschung&cp=<?=$aktplanet['planet_id'];?> ><?=loca("B_BUILDING_NEXT");?></a>';
                    }else{
                        if(s>59){
                            m=Math.floor(s/60);
                            s=s-m*60
                        ...
                        <?php
                    if ( $aktplanet['planet_id'] == $resq['sub_id'] )  echo "\">".loca("B_BUILDING_CANCEL")."</a>\"";   ?>                }
                    ;
...
<?php
            ...
            {
                if ($GlobalUser['r'.$id]) {
                    if (IsEnoughResources ( $aktplanet, $m, $k, $d, $e ) ) echo " <a href=index.php?page=buildings&session=$session&mode=Forschung&bau=$id><font color=#00FF00>".loca("BUILDINGS_RESEARCH_TO")."  $level</font></a>";
                    else echo "<font color=#FF0000>".loca("BUILDINGS_RESEARCH_TO")."  $level</font>";
                }
                else {
                    if (IsEnoughResources ( $aktplanet, $m, $k, $d, $e ) ) echo " <a href=index.php?page=buildings&session=$session&mode=Forschung&bau=$id><font color=#00FF00> ".loca("BUILDINGS_RESEARCH")." </font></a>";
                    else echo "<font color=#FF0000> ".loca("BUILDINGS_RESEARCH")." </font></a>";
                }
            }
            echo "</td></tr>";
        }
        ...
        if (!$busy) echo "<table><tr><td class=c>".loca("BUILDINGS_RESEARCH_BUILDING_REQUIRED")."</td></tr></table>";
    }
}
...
?>
      ...
      <br><?=loca("BUILDINGS_PRODUCING")?>: <div id="bx" class="z"></div>
...
b = new Array(<?=$b;?>"");
a = new Array(<?=$a;?>"");
aa = '<?=loca("BUILDINGS_ASSIGNMENTS_DONE");?>';
...
function xd() {
    ...
    for (iv = p; iv <= b.length - 2; iv++) {
        ...
        if (iv == p) {
            act = " (<?=loca("BUILDINGS_IS_PRODUCED");?>)";
        }else{
            act = "";
        }
...
<table width="530">
 ...
 <tr>
    <td class="c" ><?=loca("BUILDINGS_EXPECTED_ORDERS");?></td>
 </tr>
 <tr>
...
<?=loca("BUILDINGS_ALL_BUILDING TAKES");?>
